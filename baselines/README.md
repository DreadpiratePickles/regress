...
| Field | Why it must match across the pooled runs |
|---|---|
| `goldens_sha256` | Different criteria are different questions |
| `prompt_sha256` | The target prompt is the thing under test |
| `target_model_id` | A different model is a different measurement |
| `judge_prompt_sha256` | A different rubric grades differently |
| `judge_model_id` | Judges are not interchangeable |
